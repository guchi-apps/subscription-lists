...
import { getReadableTextColor, LABEL_COLOR_PALETTE, toLabelColor } from "@/lib/labels";
...
export function LabelManager({ initialLabels }: { initialLabels: LabelDTO[] }) {
  ...
  async function onSubmit(values: LabelFormValues) {
    ...
    if (!res.ok) {
      ...
      toast.error(typeof body?.error === "string" ? body.error : "更新に失敗しました");
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-sm font-semibold text-muted-foreground">ラベル</h2>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        「{label.name}」を削除すると、付いているすべてのサブスクからも外れます。
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>キャンセル</AlertDialogCancel>
                      <AlertDialogAction onClick={() => handleDelete(label.id)}>
                        削除する
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Controller
                ...
                name="color"
                render={({ field }) => (
                  <div className="grid grid-cols-8 gap-2" role="radiogroup" aria-label="色">
                    {LABEL_COLOR_PALETTE.map((paletteColor) => (
                      <button
                        ...
                        className={cn(
                          ...
                          field.value === paletteColor && "outline-2 outline-offset-2 outline-ring"
                        )}
                      ...
                      >
                        {field.value === paletteColor && (
                          <Check
                            className="size-4"
                            style={{ color: getReadableTextColor(paletteColor) }}
                          />
                        )}
                      </button>
                    ))}
